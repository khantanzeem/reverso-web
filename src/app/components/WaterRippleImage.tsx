...
export type WaterRippleImageProps = Partial<Params> & {
  /** Image to render with the ripple effect */
  src: string;
...
export default function WaterRippleImage({
  ...
  const paramsRef = useRef<Params>({ blueish, scale, illumination, surfaceDistortion, waterDistortion });

  // Falls back to a plain <img> if WebGL is unavailable or the texture upload
  // is blocked (e.g. the image host doesn't send CORS headers).
  ...
  useEffect(() => {
    ...
    img.onload = () => {
      ...
      try {
        ...
        // Most likely a tainted-canvas SecurityError from a cross-origin
        // image without CORS headers — fall back to a plain <img>.
        console.error("WaterRippleImage: texture upload failed, falling back to <img>", e);
        setGlFailed(true);
      }
    };

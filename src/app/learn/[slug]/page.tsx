...
export default function LearnPage({ params }: { params: { slug: string } }) {
  ...
  useEffect(() => {
    getCourseBySlug(params.slug).then(setCourse);
  }, [params.slug]);
    ...
    <div className="bg-mist py-10">
      <div className="container-x grid gap-8 lg:grid-cols-[1fr_360px]">
        <div>
          ...
          <p className="mt-1 text-sm text-ink/60">Recorded session videos for this course.</p>
          ...
          {!enrolled && (
            <div className="mt-6 flex items-center gap-3 rounded-xl border border-signal/30 bg-signal/5 p-4">
              ...
              <div className="text-sm">
                ...
                <p className="text-ink/60">Enroll in this course to watch every lesson.</p>
              </div>
              <Link href={`/courses/${course.slug}/checkout`} className="btn btn-primary ml-auto shrink-0">
                Buy Now
              </Link>
            </div>
          )}
